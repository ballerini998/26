...
def simulate_population_fixed(
    ...
    for year in years[:-1]:
        ...
        for group in ["maschi_italiani", "femmine_italiani", "maschi_stranieri", "femmine_stranieri"]:
    # Age everyone by +1 (age 1 gets age 0, ..., age 100 gets age 99)
            projections.loc[projections.index[1:], f"{group}_{next_year}"] = (
                projections.loc[projections.index[:-1], f"{group}_{year}"].values
        )

    # Open age group: add age 100 from year t on top of the shifted age 99 -> 100
        projections.loc[projections["età"] == 100, f"{group}_{next_year}"] = (
            projections.loc[projections["età"] == 99, f"{group}_{year}"].values[0]
            + projections.loc[projections["età"] == 100, f"{group}_{year}"].values[0]
            )
